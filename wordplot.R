course.name<-"Negotiation Bots"

# Convert representations
textMatrix <- quanteda::dfm(dataparse$cleantext, stem=TRUE, remove=stopwords(),
                           removePunct=TRUE,removeSymbols=TRUE, removeNumbers=TRUE)

textRating <- scale(dataparse$success)


#textMatrix<-DTMtools::DTM(text.data$body, ngrams=1:3,sparse=0.99)
